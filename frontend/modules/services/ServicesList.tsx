...
export default function ServicesList() {
  ...
  const handleSubmit = async () => {
    try {
      const payload = { ...form, product_type: 'SERVICE', status: 'ACTIVE' };
      if (editItem) {
        await api.patch(`/services/${editItem.id}`, payload);
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          <div className="px-6 py-4 border-b border-gray-100">
            <h3 className="text-lg font-bold text-gray-900">{editItem ? 'Edit' : 'New'} Service</h3>
          </div>
          <div className="p-6 space-y-5">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button onClick={handleSubmit} className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-bold text-sm transition-colors">Save</button>
            </div>
          </div>
        </div>
      )}
